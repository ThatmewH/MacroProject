...
public class DivideOperation extends LogicOperation {

    public DivideOperation(String firstVariable, String secondVariable) {
    ...
    public Serializable eval() {
        try {
            ...
            } else if (variables.get(0).getValue() instanceof Long) {
                return ((Long) variables.get(1).getValue()) / ((Long) variables.get(0).getValue());
    ...
    public static RegisteredLogicOperation registerOperation() {
        return new RegisteredLogicOperation("/", DivideOperation.class);
    }
}
